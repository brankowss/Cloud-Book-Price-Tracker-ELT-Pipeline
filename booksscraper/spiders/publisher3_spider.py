...
class PrometejSpider(scrapy.Spider):
    ...
    def parse_book_page(self, response):    
        ...
        for r in rd:
            ...
            item['author'] = r.xpath('(//div[@id="content"]//span[@itemprop="author"])[1]/text()').get() or ''
            item['book_link'] = r.xpath('(//div[@class="book-cover small"]/a)[1]/@href').get()
            item['category'] = r.xpath('//span[@class="posted_in"]/a/text()').get()
            item['old_price'] = r.xpath('//div[@id="content"]//del/text()').get() 
